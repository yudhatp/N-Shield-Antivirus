...
begin
{$IFDEF Ver100}    { added D3 support 5-26-97  KLB }
	raise EListError.Create(SListIndexError);            {  This is for version 3  }
{$ELSE}
	raise EListError.Create('Error');   { version 1 and 2         }
{$ENDIF}
end;
...
(*************************************************)
 Finalization
(*************************************************)
 For FIdx:=0 to ClassRegistry.Count-1
  do (ClassRegistry.Objects[FIdx] as tRegisterRec).Free;
 ClassRegistry.Free;
{$ENDIF}
