...
uses SysUtils, Windows, MemCheck;
...
var
  ...
  var             // This is done in a sub-routine because string variable is used and we want it finalized
    s : string;
    E: Exception;
   CS: TCallStack;  //memcheck
    t : TextFile;
  begin
   OutputFile := ExtractFilePath(ParamStr(0)) + 'exception.log';
    s:='-----------------------------------------'#13#10;
    s:=s+'N-Shield AntiVirus 2014 | Exception log'#13#10;
    s:=s+'-----------------------------------------'#13#10;
    ...
    begin                                    // thus we can access 'Exception' class
      try
        ...
        if (E is Exception) then
          //s:=s+'new exception, type '+E.ClassName+', message: '+E.Message+#13#10;
          //s:=s+(Format('%s', [DateTimeToStr(Now)]))+#13#10;
          s:=s+'Exception type '+E.ClassName+#13#10;
          s:=s+'Exception message: '+E.Message+#13#10;
      except
      end;
    end;

    {if inttostr( PER^.ExceptionCode) = '3221225477' then
    begin
    exit;
    end; }
    ...
    s:=s+        'Exception code: '+inttostr( PER^.ExceptionCode)+#13#10+
                 ...
                 TextualDebugInfoForAddress(Cardinal(PER^.ExceptionAddress))+#13#10+ //memcheck
                 CallStackTextualRepresentation(CS, '')+#13#10;            //memcheck
